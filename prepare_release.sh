...
if [[ "$2" == *"beta"* ]]
then
   # Grab last occurence of versionName and commit and replace them with {version}-{$2} in fdroid.yml and in .env
   # commit changes to master-beta
   # tag it with new version name
...
then
    ...
    echo "Invalud second argument. Must be either qa.<<number>> or beta.<<number>>"
    exit 1
fi
